...
inherit cmake-utils flag-o-matic multiprocessing wxwidgets
...
src_prepare() {
	if use doc; then
		doxygen -s -u "${S}"/doc/Doxyfiles/openthreads.doxyfile.cmake || die
		sed -e 's|DOT_NUM_THREADS.*|DOT_NUM_THREADS = '$(makeopts_jobs)'|' -i "${S}"/doc/Doxyfiles/openthreads.doxyfile.cmake || die
		doxygen -s -u "${S}"/doc/Doxyfiles/doxyfile.cmake || die
		sed -e 's|DOT_NUM_THREADS.*|DOT_NUM_THREADS = '$(makeopts_jobs)'|' -i "${S}"/doc/Doxyfiles/doxyfile.cmake || die
	fi
	default
}
